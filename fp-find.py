...
full = [int(i.strip()) & 0xFFFFFFFF for i in open(sys.argv[1]).read().split(',')]
short = [int(i.strip()) & 0xFFFFFFFF for i in open(sys.argv[2]).read().split(',')]

# don't use all 32 bits for the initial matching phase
full_20bit = [x >> 12 for x in full]
short_20bit = [x >> 12 for x in short]
